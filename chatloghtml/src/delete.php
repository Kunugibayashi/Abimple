...
if ($_SERVER['REQUEST_METHOD'] != 'POST') {
  ...
  $logLists = selectChatlogfilesById($dbhChatLogFiles, $inputParams['id']);
  if (!usedArr($logLists)) {
    $errors[] = 'ログが存在しません。';
    goto outputPage;
  }
...
// DB削除
$result = deleteChatlogfilesById($dbhChatLogFiles, $inputParams['id']);
if (!$result) {
  ...
  <?php if (isAdmin()) { /* 管理ユーザーは常に表示 */ ?>
    <div class="note-wrap">
      <p class="note">
        ...
        出力されたログファイルの内容を修正したい場合は chatloghtml/logs/ 配下のファイルを編集してください。<br>
      </p>
    </div>
  <?php } ?>
