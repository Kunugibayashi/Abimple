...
<div class="content-wrap">
  ...
  <?php if (!usedStr($success) && !usedArr($errors)) { /* 成功でもエラーでもない場合にフォームを表示 */ ?>
    ...
    <form id="delete-form" class="hidden-form" action="<?php echo h(LETTER_SEC_LINK); ?>delete.php" method="POST">
    ...
    <form id="delete-form" class="hidden-form" action="<?php echo h(LETTER_SEC_LINK); ?>inboxdelete.php" method="POST">
      <input type="hidden" name="token" value="<?php echo h(getToken()); ?>">
      <input type="hidden" name="deleteid" value="<?php echo h($inputParams['deleteid']); ?>">
    </form>
  <?php } ?>
